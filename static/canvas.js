...
(function () {
  ...
  // ── Visual tiers ──────────────────────────────────────────────────────────
  const HUB_TYPES = new Set(['Service', 'Database', 'Infrastructure', 'SystemDesign']);

  // Hub: vivid fill, white text.  Leaf: tinted fill, dark text.
  const TYPE_PAL = {
    Service:        { fill:'#6366f1', text:'#fff', stroke:'#4f46e5',  lFill:'#eef2ff', lText:'#3730a3', lStroke:'#818cf8' },
    Database:       { fill:'#10b981', text:'#fff', stroke:'#059669',  lFill:'#d1fae5', lText:'#065f46', lStroke:'#34d399' },
    Infrastructure: { fill:'#f59e0b', text:'#fff', stroke:'#d97706',  lFill:'#fef3c7', lText:'#92400e', lStroke:'#fbbf24' },
    File:           { fill:'#3b82f6', text:'#fff', stroke:'#2563eb',  lFill:'#eff6ff', lText:'#1e40af', lStroke:'#93c5fd' },
    SystemDesign:   { fill:'#8b5cf6', text:'#fff', stroke:'#7c3aed',  lFill:'#f5f3ff', lText:'#5b21b6', lStroke:'#a78bfa' },
  };
  const PAL_DEF = { fill:'#6b7280', text:'#fff', stroke:'#4b5563', lFill:'#f9fafb', lText:'#374151', lStroke:'#d1d5db' };

  function pal(d)   { return TYPE_PAL[d.label] || PAL_DEF; }
  function isHub(d) { return HUB_TYPES.has(d.label); }
  ...
  // ── State ─────────────────────────────────────────────────────────────────
  let _svg = null, _zoomG = null, _zoom = null, _sim = null;
  let _nodesData = [], _edgesData = [];
  let _parentMap = {}, _groupIds = new Set(), _byId = {};
  ...
  function bootstrap() {
    ...
    [['arr', '#c4c8d4'], ['arr-add', '#22c55e'], ['arr-del', '#ef4444']].forEach(([id, col]) =>
    ...
    ['groups', 'links', 'nodes'].forEach(cls => _zoomG.append('g').attr('class', `layer-${cls}`));

    _zoom = d3.zoom().scaleExtent([0.04, 4]).on('zoom', evt => {
      _zoomG.attr('transform', evt.transform);
      schedMinimap();
    });
    _svg.call(_zoom);

    _svg.on('click', evt => { if (evt.target === _svg.node()) _clearSearch(true); });

    const wrap = document.getElementById('canvas-wrapper');
    if (wrap) { wrap.appendChild(_buildToolbar()); wrap.appendChild(_buildMinimap()); }
  }
  ...
  function render(nodes, edges, diffData) {
    ...
    _parentMap = {};
    _groupIds  = new Set();
    edges.forEach(e => { if (e.rel === 'MEMBER_OF') { _parentMap[e.src] = e.dst; _groupIds.add(e.dst); } });
    ...
    const eAdd    = new Set((diffData?.added_edges   || []).map(e => `${e.src}|${e.dst}`));
    const eDel    = new Set((diffData?.deleted_edges || []).map(e => `${e.src}|${e.dst}`));
    ...
    renderNodes.forEach(n => {
      n._m = nodeMetrics(n);
      if (n.x == null) { n.x = W / 2 + (Math.random() - 0.5) * 400; }
      if (n.y == null) { n.y = H / 2 + (Math.random() - 0.5) * 300; }
    });

    const links = edges
      .filter(e => e.rel !== 'MEMBER_OF' && _byId[e.src] && _byId[e.dst]
                   && !_groupIds.has(e.src) && !_groupIds.has(e.dst))
    ...
    groupNodes.forEach(gn => {
      ...
      g.append('rect').attr('class', 'grp-rect').attr('rx', 12)
        .attr('fill', 'rgba(148,163,184,0.09)')
        .attr('stroke', '#cbd5e1').attr('stroke-width', 1.5)
        .attr('stroke-dasharray', '6,4');
      g.append('text').attr('class', 'grp-label')
        .attr('text-anchor', 'middle')
        .attr('fill', '#b0b5c0').attr('font-size', '8.5px')
        .attr('font-family', 'Consolas,monospace').attr('font-weight', '700')
    ...
    const linkEls = lLayer.selectAll('g.lnk')
      .data(links, d => `${d.src}|${d.dst}|${d.rel}`)
      .join('g').attr('class', 'lnk');

    linkEls.append('path').attr('fill', 'none')
      .attr('stroke', d => eAdd.has(`${d.src}|${d.dst}`) ? '#22c55e'
                         : eDel.has(`${d.src}|${d.dst}`) ? '#ef4444' : '#d1d5db')
      .attr('stroke-width', 1)
      .attr('marker-end', d => eAdd.has(`${d.src}|${d.dst}`) ? 'url(#arr-add)'
                              : eDel.has(`${d.src}|${d.dst}`) ? 'url(#arr-del)' : 'url(#arr)');

    linkEls.append('text')
      .attr('text-anchor', 'middle').attr('fill', '#c0c3cc')
      .attr('font-size', '8px').attr('font-family', 'Consolas,monospace')
      .attr('pointer-events', 'none').text(d => d.rel || '');
    ...
    const nodeEls = nLayer.selectAll('g.nd')
      .data(renderNodes, d => d.id)
      .join('g').attr('class', d => `nd ${d.label || ''}`)
      .attr('data-id', d => d.id)
    ...
    nodeEls.each(function (d) {
      ...
      let sw   = m.hub ? 2 : 1.5;
      if (diffAdd.has(d.id)) { fill = '#dcfce7'; stk = '#22c55e'; }
      if (diffDel.has(d.id)) { fill = '#fee2e2'; stk = '#ef4444'; }
      if (diffMod.has(d.id)) { fill = '#fef9c3'; stk = '#f59e0b'; }

      sel.append('rect').attr('class', 'nd-rect')
        .attr('x', -m.w / 2).attr('y', -m.h / 2)
        .attr('width', m.w).attr('height', m.h).attr('rx', m.rx)
        .attr('fill', fill).attr('stroke', stk).attr('stroke-width', sw);

      const iconFn = ICONS[d.label];
      if (iconFn) {
        const ic = m.hub ? 'rgba(255,255,255,0.9)' : p.lStroke;
        sel.append('g')
    ...
    nodeEls.on('click', (ev, d) => {
      ...
      nLayer.selectAll('.nd-rect').each(function (n) {
        const m = n._m, p = pal(n);
        d3.select(this).attr('stroke', m.hub ? p.stroke : p.lStroke).attr('stroke-width', m.hub ? 2 : 1.5);
      });
      d3.select(ev.currentTarget).select('.nd-rect')
        .attr('stroke', '#6366f1').attr('stroke-width', d._m.hub ? 3 : 2.5);
      if (window.CortexUI?.onNodeClick) window.CortexUI.onNodeClick(d.id);
    });

    nodeEls
      .on('mouseenter', function () { d3.select(this).select('.nd-rect').attr('filter', 'brightness(0.94)'); })
      .on('mouseleave', function () { d3.select(this).select('.nd-rect').attr('filter', null); });

    // ── Simulation ────────────────────────────────────────────────────────────
    _sim = d3.forceSimulation(renderNodes)
      .force('link', d3.forceLink(links).id(d => d.id)
        ...
        .strength(0.6))
      .force('charge',    d3.forceManyBody().strength(d => isHub(d) ? -700 : -220))
    ...
    // ── Tick ─────────────────────────────────────────────────────────────────
    _sim.on('tick', () => {
      nodeEls.attr('transform', d => `translate(${d.x.toFixed(1)},${d.y.toFixed(1)})`);
      ...
      linkEls.each(function (d) {
        ...
        lk.select('path').attr('d', `M${sp.x.toFixed(1)},${sp.y.toFixed(1)}L${tp.x.toFixed(1)},${tp.y.toFixed(1)}`);
        lk.select('text')
      ...
      Object.keys(gEls).forEach(gid => {
        ...
        const g  = gEls[gid];
        g.select('.grp-rect').attr('x', x0).attr('y', y0).attr('width', x1 - x0).attr('height', y1 - y0);
        g.select('.grp-label').attr('x', (x0 + x1) / 2).attr('y', y0 + 16);
      });
    });

    _sim.on('end', () => { fitView(); schedMinimap(); });
  }
  ...
  function _cohesionForce(nodes) {
    return function (alpha) {
      const gs = {};
      nodes.forEach(n => { const g = _parentMap[n.id]; if (g) (gs[g] = gs[g] || []).push(n); });
      Object.values(gs).forEach(ms => {
        if (ms.length < 2) return;
        const cx = d3.mean(ms, n => n.x), cy = d3.mean(ms, n => n.y);
        ms.forEach(n => { n.vx -= (n.x - cx) * alpha * 0.07; n.vy -= (n.y - cy) * alpha * 0.07; });
      });
    };
  }
  ...
  function fitView() {
    ...
    try {
      ...
      const pad = 48;
      const k   = Math.min((W - pad * 2) / bb.width, (H - pad * 2) / bb.height, 1.5);
      _svg.transition().duration(500).call(
        _zoom.transform,
        d3.zoomIdentity.translate((W - bb.width * k) / 2 - bb.x * k, (H - bb.height * k) / 2 - bb.y * k).scale(k)
      );
    } catch (_) {}
  }
  ...
  function focusNode(nodeId) {
    ...
    const el = document.getElementById('graph-canvas');
    if (!el) return;
    const W = el.clientWidth, H = el.clientHeight, k = 2;
    ...
    _zoomG.selectAll('.nd-rect').each(function (d) {
      const m = d._m, p = pal(d);
      d3.select(this).attr('stroke', m.hub ? p.stroke : p.lStroke).attr('stroke-width', m.hub ? 2 : 1.5);
    });
    _zoomG.selectAll('g.nd').filter(function () { return this.dataset.id === nodeId; })
      .select('.nd-rect').attr('stroke', '#6366f1').attr('stroke-width', 3);
  }
  ...
  function highlightSearch(query) {
    ...
    _zoomG.selectAll('g.nd').style('opacity', function () {
      ...
      const n  = _byId[id];
      return n && ((n.name || '').toLowerCase().includes(q) || id.toLowerCase().includes(q)) ? 1 : 0.28;
    });
  }
  ...
  function _buildMinimap() {
    ...
    Object.assign(mm.style, {
      ...
      boxShadow: '0 2px 12px rgba(0,0,0,0.06)', pointerEvents: 'none',
    });
    const lbl = document.createElement('div');
    lbl.textContent = 'OVERVIEW';
    Object.assign(lbl.style, { position: 'absolute', top: '5px', left: '8px', fontSize: '7px', color: '#9ca3af', fontFamily: 'Consolas,monospace', letterSpacing: '0.1em', fontWeight: '700', zIndex: '1' });
    const img = document.createElement('img');
    img.id = 'cortex-minimap-img';
    Object.assign(img.style, { position: 'absolute', top: '18px', left: '0', width: '100%', height: 'calc(100% - 18px)', objectFit: 'contain' });
    mm.appendChild(lbl);
    mm.appendChild(img);
    return mm;
  }
    ...
    _mmTimer = setTimeout(() => {
      ...
      try {
        ...
        if (bb.width > 0) {
          ...
          bg.setAttribute('x', bb.x - 20); bg.setAttribute('y', bb.y - 20);
          bg.setAttribute('width', bb.width + 40); bg.setAttribute('height', bb.height + 40);
  ...
  function _buildToolbar() {
    ...
    Object.assign(bar.style, { position: 'absolute', bottom: '14px', left: '14px', display: 'flex', flexDirection: 'column', gap: '4px', zIndex: '10' });
    const s = 'fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"';
    function btn(title, ico, fn) {
      const b = document.createElement('button');
      b.title = title; b.innerHTML = ico;
      Object.assign(b.style, { width: '32px', height: '32px', borderRadius: '10px', background: 'rgba(255,255,255,0.92)', border: '1px solid #e5e7eb', color: '#6b7280', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', transition: 'all 0.12s' });
      b.onmouseenter = () => { b.style.color = '#111'; b.style.borderColor = '#d1d5db'; };
      b.onmouseleave = () => { b.style.color = '#6b7280'; b.style.borderColor = '#e5e7eb'; };
      b.onclick = fn;
      return b;
    }
    bar.appendChild(btn('Fit view', `<svg viewBox="0 0 24 24" width="15" height="15" ${s}><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/></svg>`, fitView));
    bar.appendChild(btn('Zoom in',  `<svg viewBox="0 0 24 24" width="15" height="15" ${s}><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><line x1="11" y1="8" x2="11" y2="14"/><line x1="8" y1="11" x2="14" y2="11"/></svg>`, () => _svg?.transition().duration(200).call(_zoom?.scaleBy, 1.4)));
    bar.appendChild(btn('Zoom out', `<svg viewBox="0 0 24 24" width="15" height="15" ${s}><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><line x1="8" y1="11" x2="14" y2="11"/></svg>`, () => _svg?.transition().duration(200).call(_zoom?.scaleBy, 0.7)));
    return bar;
  }
  ...
  function screenshot() {
    ...
    try {
      ...
      clone.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
      const bg = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
      bg.setAttribute('width', '100%'); bg.setAttribute('height', '100%'); bg.setAttribute('fill', '#f4f4f8');
      clone.insertBefore(bg, clone.firstChild);
